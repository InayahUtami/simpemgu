...
export default function DataPerKecamatan() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'districts' | 'demographics' | 'growth' | 'map' | 'data' | 'statistics' | 'profile' | 'hybrid-ahp-ahc'>('data');
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((item: Kecamatan) =>
        ...
        (item.kode_kecamatan && item.kode_kecamatan.includes(searchTerm))
      );
    }
    ...
    try {
      if (modalMode === 'add') {
        ...
        if (result.success) {
          ...
          setNotifMsg(result.error || 'Gagal menambah data!');
          setShowNotif(true);
          setTimeout(() => setShowNotif(false), 2000);
        }
        ...
        if (result.success) {
          ...
          setNotifMsg(result.error || 'Gagal mengupdate data!');
          setShowNotif(true);
          setTimeout(() => setShowNotif(false), 2000);
        }
      }
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      if (result.success) {
        ...
        setNotifMsg(result.error || 'Gagal menghapus data!');
        setShowNotif(true);
        setTimeout(() => setShowNotif(false), 2000);
      }
    } catch (error) {
  ...
  const exportToCSV = () => {
    const headers = ['No', 'Nama Kecamatan', 'Kode Kecamatan'];
    ...
    link.download = 'data_kecamatan.csv'; // diganti ke .csv dari .xls agar sesuai format
    link.click();
  };

  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom right, #dbeafe 0%, #e0e7ff 33%, #f3e8ff 66%, #fdf2f8 100%)' }}>
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 80px)' }}>
        ...
        <main style={{ 
          ...
          {/* Menu profil di ujung kanan atas */}
          {!isMobile && (
          ...
          <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
            <div style={{ position: 'relative', flex: '1' }}>
              ...
              <input
                type="text"
                placeholder="Cari nama kecamatan atau kode pos..."
                value={searchTerm}
            ...
            <div style={{ position: 'relative' }}>
              ...
              <select
                value={filterBy}
                onChange={(e) => setFilterBy(e.target.value as 'semua' | 'nama_asc' | 'nama_desc')}
                style={{
                  padding: '12px 12px 12px 44px',
                  border: '2px solid #e5e7eb',
                  borderRadius: '8px',
                      ...
                      <td style={{ padding: '12px' }}>
                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                          ...
                          <button
                            ...
                            style={{
                              padding: '8px',
                              backgroundColor: '#ef4444', // Diubah menjadi merah agar semantik "Hapus"
                              color: 'white',
                              border: 'none',
                              borderRadius: '6px',
                              cursor: 'pointer',
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                  <h2 style={{
                    ...
                    <MapPin style={{ width: '28px', height: '28px', color: '#1e3a8a' }} />
                  ...
                  <div style={{ display: 'grid', gap: '16px' }}>
                    <div style={{ padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
                      ...
                      <div style={{ display: 'grid', gap: '8px' }}>
                        {selectedKecamatan.kode_kecamatan && (
                          <p style={{ margin: '0', color: '#000000' }}>
                            <strong>Kode Kecamatan:</strong> {selectedKecamatan.kode_kecamatan}
                          </p>
                        )}
                        <p style={{ margin: '0', color: '#000000' }}>
