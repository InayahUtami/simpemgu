...
interface RombelPerKecamatanRow {
  no: number;
  kecamatan_id: number;
  kecamatan: string;
  tahun: string;
...
export default function DataRombelPage() {
  ...
  const [rows, setRows] = useState<RombelPerKecamatanRow[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'Gagal memuat data rombel');
      }
      ...
      const sourceRows: RombelPerKecamatanRow[] = Array.isArray(result.data) ? result.data : [];
      ...
      let syncedRows: RombelPerKecamatanRow[] = sourceRows;
  ...
  const groupedRows = useMemo(() => {
    const map = new Map<string, { kecamatan_id: number; kecamatan: string; perYear: Record<string, RombelPerKecamatanRow> }>();
  ...
  const totalRombel = useMemo(() => {
    return filteredRows.reduce((acc, row) => acc + Number(row.total_rombel || 0), 0);
  }, [filteredRows]);
  ...
  const totalRombelByYear = useMemo(() => {
    return availableYears.map((year) =>
      filteredRows.reduce((acc, row) => (row.tahun === year ? acc + Number(row.total_rombel || 0) : acc), 0)
    );
  }, [filteredRows, availableYears]);
  ...
  const exportCSV = () => {
    const headers = ['No', 'Kecamatan', 'Tahun', 'Jumlah Sekolah', 'Total Rombel'];
        ...
        >
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
            ...
            <div style={{ backgroundColor: '#f3f4f6', borderRadius: '8px', marginBottom: '16px', padding: '12px 16px', color: '#374151', fontSize: '12px' }}>
              Menampilkan {filteredRows.length} baris data rombel {searchTerm && `dengan pencarian "${searchTerm}"`}.
            </div>
              ...
              <table style={{ minWidth: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#e5e7eb' }}>
                    ...
                    <th colSpan={Math.max(pagedYears.length, 1)} style={{ textAlign: 'center', padding: '12px', color: 'white', border: '1px solid #93c5fd', fontSize: '13px' }}>Total Rombel</th>
                  </tr>
                  <tr style={{ background: '#e5e7eb' }}>
                    ...
                    {pagedYears.map((year, index) => (
                      <th key={`rombel-${year}-${index}`} style={{ textAlign: 'center', padding: '12px', color: 'white', border: '1px solid #93c5fd', fontSize: '13px' }}>
                        {year === '-' ? '-' : `Tahun ${year}`}
                      </th>
                    ))}
                  </tr>
                  ...
                  {isLoading ? (
                    [...Array(5)].map((_, i) => (
                      <tr key={`skeleton-${i}`} style={{ backgroundColor: i % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                        ...
                        {pagedYears.map((year, yearIndex) => (
                          <td key={`skeleton-rombel-${year}-${yearIndex}`} style={{ padding: '12px', border: '1px solid #e5e7eb', textAlign: 'right' }}>
                            <div style={{
                              height: '16px',
                              background: 'linear-gradient(90deg, #e5e7eb 25%, #f3f4f6 50%, #e5e7eb 75%)',
                              backgroundSize: '200% 100%',
                    ...
                    paginatedGroupedRows.map((row, index) => (
                      <tr key={`${row.kecamatan_id}-${index}`} style={{ backgroundColor: index % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                        ...
                        <td 
                          onClick={() => router.push(`/admin/data/per-kecamatan/${row.kecamatan_id}?type=rombel&name=${encodeURIComponent(row.kecamatan)}`)}
                        ...
                        {pagedYears.map((year) => (
                          <td key={`rombel-${row.kecamatan_id}-${year}`} style={{ padding: '12px', textAlign: 'right', border: '1px solid #e5e7eb', fontSize: '13px' }}>
                            {Number(row.perYear[year]?.total_rombel || 0).toLocaleString('id-ID')}
                          </td>
                        ))}
                      </tr>
                    ))
                    ...
                    <tr>
                      <td colSpan={2 + (Math.max(pagedYears.length, 1) * 2)} style={{ padding: '28px 12px', border: '1px solid #e5e7eb', textAlign: 'center', color: '#6b7280', fontSize: '14px', fontWeight: 600 }}>
                ...
                {!isLoading && filteredRows.length > 0 && (
                  <tfoot>
                    <tr style={{ background: '#e5e7eb' }}>
                      ...
                      {pagedRombelTotals.map((value, index) => (
                        <td key={`total-rombel-${index}`} style={{ padding: '12px', textAlign: 'right', fontWeight: 800, border: '1px solid #e5e7eb', fontSize: '13px', background: '#e5e7eb' }}>
                          {value.toLocaleString('id-ID')}
                        </td>
                      ))}
                    </tr>
