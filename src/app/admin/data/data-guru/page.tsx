...
import { UserSquare2, Search, Download } from 'lucide-react';
import Sidebar from '../../components/Sidebar';
import ProfileMenu from '../../components/ProfileMenu';
import { useSidebar } from '../../components/SidebarContext';

interface GuruPerKecamatanRow {
  no: number;
  kecamatan_id: number;
  kecamatan: string;
  tahun: string;
...
export default function DataGuruPage() {
  ...
  const [rows, setRows] = useState<GuruPerKecamatanRow[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [response, kecamatanResponse] = await Promise.all([
        fetch('/api/data/guru-per-kecamatan', { cache: 'no-store' }),
      ...
      const sourceRows: GuruPerKecamatanRow[] = Array.isArray(result.data) ? result.data : [];
      ...
      let syncedRows: GuruPerKecamatanRow[] = sourceRows;
  ...
  const groupedRows = useMemo(() => {
    const map = new Map<string, { kecamatan_id: number; kecamatan: string; perYear: Record<string, GuruPerKecamatanRow> }>();
  ...
  const totalGuru = useMemo(() => {
    return filteredRows.reduce((acc, row) => acc + Number(row.total_guru || 0), 0);
  }, [filteredRows]);

  const totalGuruByYear = useMemo(() => {
    return availableYears.map((year) =>
      filteredRows.reduce((acc, row) => (row.tahun === year ? acc + Number(row.total_guru || 0) : acc), 0)
    );
  ...
  const exportCSV = () => {
    const headers = ['No', 'Kecamatan', 'Tahun', 'Total Guru'];
        ...
        >
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
            <div style={{ marginBottom: '32px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '8px' }}>
                <div style={{ padding: '12px', borderRadius: '12px', backgroundColor: '#1e3a8a20', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <UserSquare2 style={{ width: '32px', height: '32px', color: '#1e3a8a' }} />
                </div>
                <div>
            ...
            <div style={{ backgroundColor: '#f3f4f6', borderRadius: '8px', marginBottom: '16px', padding: '12px 16px', color: '#374151', fontSize: '12px' }}>
              Menampilkan {filteredRows.length} baris data guru {searchTerm && `dengan pencarian "${searchTerm}"`}.
            </div>
              ...
              <table style={{ minWidth: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #93c5fd 100%)' }}>
                    ...
                    <th colSpan={Math.max(pagedYears.length, 1)} style={{ textAlign: 'center', padding: '12px', color: 'white', border: '1px solid #93c5fd', fontSize: '13px' }}>Total Guru</th>
                  </tr>
                  <tr style={{ background: 'linear-gradient(135deg, #1e3a8a 0%, #93c5fd 100%)' }}>
                    {pagedYears.map((year, index) => (
                      <th key={`guru-${year}-${index}`} style={{ textAlign: 'center', padding: '12px', color: 'white', border: '1px solid #93c5fd', fontSize: '13px' }}>
                        {year === '-' ? '-' : `Tahun ${year}`}
                      </th>
                    ))}
                  </tr>
                  ...
                  {isLoading ? (
                    [...Array(5)].map((_, i) => (
                      <tr key={`skeleton-${i}`} style={{ backgroundColor: i % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                        ...
                        {pagedYears.map((year, yearIndex) => (
                          <td key={`skeleton-guru-${year}-${yearIndex}`} style={{ padding: '12px', border: '1px solid #e5e7eb', textAlign: 'right' }}>
                            <div style={{
                              height: '16px',
                              background: 'linear-gradient(90deg, #e5e7eb 25%, #f3f4f6 50%, #e5e7eb 75%)',
                              backgroundSize: '200% 100%',
                    ...
                    paginatedGroupedRows.map((row, index) => (
                      <tr key={`${row.kecamatan_id}-${index}`} style={{ backgroundColor: index % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                        ...
                        <td 
                          onClick={() => router.push(`/admin/data/per-kecamatan/${row.kecamatan_id}?type=guru&name=${encodeURIComponent(row.kecamatan)}`)}
                        ...
                        {pagedYears.map((year) => (
                          <td key={`guru-${row.kecamatan_id}-${year}`} style={{ padding: '12px', textAlign: 'right', border: '1px solid #e5e7eb', fontSize: '13px' }}>
                            {Number(row.perYear[year]?.total_guru || 0).toLocaleString('id-ID')}
                          </td>
                        ))}
                      </tr>
                    ))
                ...
                {!isLoading && filteredRows.length > 0 && (
                  <tfoot>
                    <tr style={{ background: '#e5e7eb' }}>
                      ...
                      {pagedGuruTotals.map((value, index) => (
                        <td key={`total-guru-${index}`} style={{ padding: '12px', textAlign: 'right', fontWeight: 800, border: '1px solid #e5e7eb', fontSize: '13px', background: '#e5e7eb' }}>
