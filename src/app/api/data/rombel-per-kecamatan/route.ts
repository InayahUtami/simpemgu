 ...
 * Menampilkan jumlah rombel (rombongan belajar/kelas) per kecamatan per tahun
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const yearsQuery = tahun
      ? 'SELECT DISTINCT tahun FROM view_rombel_per_kecamatan WHERE tahun = ? ORDER BY tahun'
      : 'SELECT DISTINCT tahun FROM view_rombel_per_kecamatan ORDER BY tahun';
    ...
    if ((yearRows as any[]).length === 0) {
      ...
      let query = `
        SELECT
          ...
          COALESCE(v.total_rombel, 0) AS total_rombel
        FROM kecamatan k
        CROSS JOIN (
        ...
        LEFT JOIN view_rombel_per_kecamatan v
