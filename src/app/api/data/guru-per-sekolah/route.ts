 ...
 * Menampilkan jumlah guru per sekolah dalam satu kecamatan per tahun
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const connection = await getDbConnection();
    const [existing] = await connection.query('SELECT id FROM data_guru_sekolah WHERE kecamatan_id = ? AND tahun = ? LIMIT 1', [kecamatan_id, tahun]);
    ...
    for (const school of schools as any[]) {
      await connection.query('INSERT INTO data_guru_sekolah (sekolah_id, kecamatan_id, nama_sekolah, tahun, jumlah_guru) VALUES (?, ?, ?, ?, 0)', [school.id, kecamatan_id, school.nama_sekolah, tahun]);
    }
    await connection.end();
...
export async function PUT(request: NextRequest) {
  try {
    ...
    for (const u of updates) {
      ...
      await connection.query('UPDATE data_guru_sekolah SET jumlah_guru = ?, updated_at = NOW() WHERE kecamatan_id = ? AND nama_sekolah = ? AND tahun = ?', [Number(value) || 0, kecamatan_id, nama_sekolah, tahun]);
    }
    await connection.end();
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!kecamatan_id || !tahun) return NextResponse.json({ success: false, error: 'kecamatan_id dan tahun wajib diisi' }, { status: 400 });
    const connection = await getDbConnection();
    await connection.query('DELETE FROM data_guru_sekolah WHERE kecamatan_id = ? AND tahun = ?', [kecamatan_id, tahun]);
    await connection.end();
