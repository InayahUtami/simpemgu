 ...
 * Menampilkan jumlah siswa per kecamatan per tahun
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const yearsQuery = tahun
      ? 'SELECT DISTINCT tahun FROM view_siswa_per_kecamatan WHERE tahun = ? ORDER BY tahun'
      : 'SELECT DISTINCT tahun FROM view_siswa_per_kecamatan ORDER BY tahun';
    ...
    if ((yearRows as any[]).length === 0) {
      ...
      let query = `
        SELECT
          ...
          COALESCE(v.total_siswa, 0) AS total_siswa
        FROM kecamatan k
        CROSS JOIN (
        ...
        LEFT JOIN view_siswa_per_kecamatan v
