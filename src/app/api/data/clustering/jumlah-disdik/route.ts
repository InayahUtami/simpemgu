...
// Matriks AHP sama seperti di notebook fik.ipynb
const AHP_MATRIX = [
  [1, 1, 1, 1, 1 / 3],
  [1, 1, 1, 1, 1 / 3],
  [1, 1, 1, 1, 1 / 3],
...
const AHP_WEIGHTS_ARRAY = getAhpWeights(AHP_MATRIX);

const AHP_WEIGHTS = {
  penduduk: AHP_WEIGHTS_ARRAY[0],
  guru: AHP_WEIGHTS_ARRAY[1],
  siswa: AHP_WEIGHTS_ARRAY[2],
  rombel: AHP_WEIGHTS_ARRAY[3],
  rasio: AHP_WEIGHTS_ARRAY[4],
};

// Bobot untuk weighted Euclidean distance
const VARIABLE_WEIGHTS = [
  AHP_WEIGHTS.penduduk,
  AHP_WEIGHTS.guru,
  AHP_WEIGHTS.siswa,
  AHP_WEIGHTS.rombel,
  AHP_WEIGHTS.rasio
...
function agglomerativeClustering(dist: number[][], points: number[][], k: number, linkage: LinkageType): number[] {
  let clusters: number[][] = dist.map((_, idx) => [idx]);
  while (clusters.length > k) {
    ...
    for (let i = 0; i < clusters.length; i++) {
      for (let j = i + 1; j < clusters.length; j++) {
        const d = clusterDistance(clusters[i], clusters[j], dist, linkage, points);
        if (d < bestDist) {
          bestDist = d;
          bestI = i;
          bestJ = j;
  ...
  const labels = Array(dist.length).fill(0);
...
export async function GET(request: Request) {
  try {
    ...
    const targetRatio = Number(searchParams.get('targetRatio') || '20');

    // Make rasio more dominant so the resulting clusters follow service burden more closely.
    const dominantRasioWeight = numberOfClusters === 3 ? 0.75 : 0.65;
    const remainingWeight = (1 - dominantRasioWeight) / 4;
    const dynamicWeights = [remainingWeight, remainingWeight, remainingWeight, remainingWeight, dominantRasioWeight];

    console.log('[API Clustering Disdukcapil] Request received:', { tahun, numberOfClusters });
    ...
    if (!Number.isFinite(targetRatio) || targetRatio <= 0) {
      return NextResponse.json(
        { success: false, error: 'targetRatio harus angka positif' },
        { status: 400 }
      );
    }

    // Ambil data variabel pemerataan guru per kecamatan (sumber penduduk DISDUKCAPIL)
    const results: any = await query(`
      SELECT 
        k.id,
        k.nomor,
        k.nama as kecamatan,
        COALESCE(jpd.jumlah_penduduk, 0) as jumlah_penduduk,
        COALESCE(vg.total_guru, 0) as total_guru,
        COALESCE(vs.total_siswa, 0) as total_siswa,
        COALESCE(vr.total_rombel, 0) as total_rombel
      FROM kecamatan k
      LEFT JOIN jumlah_penduduk_disdukcapil jpd ON k.id = jpd.kecamatan_id AND jpd.tahun = ?
      LEFT JOIN (
    ...
    const schoolRows: any = await query(`
    ...
    schoolRows.forEach((row: any) => {
      const arr = schoolMap.get(row.kecamatan_id) || [];
      arr.push(row.nama_sekolah);
      schoolMap.set(row.kecamatan_id, arr);
    });

    console.log('[API Clustering Disdukcapil] Query results:', { 
      totalRows: results.length, 
      sampleData: results.slice(0, 3) 
    });

    // Filter hanya kecamatan yang punya data lengkap multivariabel
    const validResults = results.filter((row: any) =>
    ...
    console.log('[API Clustering Disdukcapil] Valid results after filtering:', { 
      validCount: validResults.length,
      validData: validResults.map((r: any) => ({ kecamatan: r.kecamatan, jumlah: r.jumlah_penduduk })) 
    });
    ...
    if (validResults.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: `Tidak ada data lengkap multivariabel DISDUKCAPIL untuk tahun ${tahun}`,
    ...
    // Validasi: jumlah data harus >= jumlah cluster
    if (validResults.length < numberOfClusters) {
      return NextResponse.json(
        { 
          success: false, 
          error: `Jumlah data (${validResults.length} kecamatan) kurang dari jumlah cluster yang diminta (${numberOfClusters}). Kurangi jumlah cluster.` 
        },
        { status: 400 }
      );
    }
    ...
    const weightedPoints = normalizedPoints.map((point) => 
      point.map((val, idx) => val * dynamicWeights[idx])
    );
    ...
    const labels = agglomerativeClustering(distanceMatrix, weightedPoints, numberOfClusters, linkage);
    ...
    for (let kk = 2; kk <= maxK; kk++) {
      const labelsK = agglomerativeClustering(distanceMatrix, weightedPoints, kk, linkage);
    ...
    const clusters = Object.keys(clusterStats)
      ...
      .map((clusterKey, index) => {
        ...
        // Kategorisasi dinamis berdasarkan rasio guru-siswa (seperti notebook)
        let category = '';
        let color = '';
        
        if (numberOfClusters === 2) {
          if (index === 0) {
            category = 'Kelebihan Guru';
            color = '#87CEFA';
          } else {
            category = 'Kekurangan Guru';
            color = '#dc2626';
          }
          ...
          if (index === 0) {
            ...
            category = 'Kelebihan Guru';
            color = '#87CEFA';
          } else if (index === Math.floor(numberOfClusters / 2)) {
            category = 'Ideal';
            color = '#2ca02c';
          } else if (index === numberOfClusters - 2) {
            category = 'Kekurangan Guru';
            color = '#FFB6C1';
          } else if (index === numberOfClusters - 1) {
            category = 'Sangat Kekurangan Guru';
            color = '#d62728';
          } else {
            category = 'Transisi';
            color = '#808080';
        ...
        return {
          ...
          kecamatanList: stats.items.map((item: any) => {
            ...
            let kondisiPemerataan = '';
            if (rasioSiswaGuru < 18) {
              kondisiPemerataan = 'Kelebihan Guru';
            } else if (rasioSiswaGuru >= 18 && rasioSiswaGuru <= 20) {
              kondisiPemerataan = 'Seimbang';
            } else {
    ...
    console.error('[API Clustering Disdukcapil] ERROR:', error);
    console.error('[API Clustering Disdukcapil] Error stack:', error instanceof Error ? error.stack : 'No stack trace');
    return NextResponse.json(
      { 
        success: false, 
        error: 'Gagal melakukan clustering', 
